...
export default function CompoundDetail() {
  ...
  const { compound, bbb, cyp2e1, cyp450 } = result;

  const topCyp = useMemo(() => {
    const list = cyp450?.top ?? cyp450?.isoforms ?? [];
    return list.slice(0, 10);
  }, [cyp450]);

  const cyp2e1InFamily = useMemo(() => {
    return (cyp450?.isoforms ?? []).find(x => x.isoform === "CYP2E1") ?? null;
  }, [cyp450]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="card-glow bg-card border-border">
            <CardContent className="p-5">
              ...
              {!cyp450 ? (
                <p className="text-sm text-muted-foreground">
                  CYP450 family estimate not available for this result.
                </p>
              ) : (
                <div className="space-y-2">
                  {topCyp.map(item => {
                    ...
                    const hasEvidence =
                      Boolean(best) || (ev?.evidenceCount ?? 0) > 0;

                    const chemblCompoundUrl = cyp450?.evidenceMeta
                      ?.moleculeChEMBLId
                      ? `https://www.ebi.ac.uk/chembl/explore/compound/${cyp450.evidenceMeta.moleculeChEMBLId}`
                      : null;

                    const chemblAssayUrl = best?.assayChEMBLId
                      ? `https://www.ebi.ac.uk/chembl/explore/assay/${best.assayChEMBLId}`
                      : null;

                    const chemblDocUrl = best?.documentChEMBLId
                      ? `https://www.ebi.ac.uk/chembl/explore/document/${best.documentChEMBLId}`
                      : null;
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            {hasEvidence ? (
                              <div className="text-[11px] text-muted-foreground/80 mt-0.5">
                                {best ? (
                                  <>
                                    Best evidence: {best.standardType}{" "}
                                    {best.relation ?? ""} {best.standardValue}{" "}
                                    {best.standardUnits}
                                    {best.pchemblValue != null
                                      ? ` (pChEMBL ${best.pchemblValue})`
                                      : ""}
                                  </>
                                ) : (
                                  <>Evidence count: {ev?.evidenceCount ?? 0}</>
                                )}
                              </div>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            <LevelBadge level={item.potential} />
                          </div>
                        </div>
                        ...
                        {hasEvidence ? (
                          <div className="mt-1 text-[11px] text-muted-foreground/70">
                            Evidence-first (ChEMBL). Score is still a model
                            helper.
                          </div>
                        ) : (
                          <div className="mt-1 text-[11px] text-muted-foreground/60">
                            No ChEMBL evidence found → showing model estimate.
                          </div>
                        )}
                      </div>
                    );
                  ...
                  {/* Explicit CYP2E1 (family) hint */}
                  {cyp2e1InFamily ? (
                    <div className="pt-2 text-[11px] text-muted-foreground">
                      CYP2E1 (family estimate): {cyp2e1InFamily.score}/10 ·{" "}
                      {cyp2e1InFamily.potential}
                    </div>
                  ) : null}
                </div>
              )}
              ...
              <div className="mt-4 text-[11px] text-muted-foreground/70">
                {cyp450?.evidenceMeta ? (
                  <>
                    Evidence source: {cyp450.evidenceMeta.source} (molecule:{" "}
                    <a
                      className="text-primary hover:underline"
                      href={`https://www.ebi.ac.uk/chembl/explore/compound/${cyp450.evidenceMeta.moleculeChEMBLId}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      {cyp450.evidenceMeta.moleculeChEMBLId}
                    </a>
                    ). Fetched at{" "}
                    {new Date(cyp450.evidenceMeta.fetchedAt).toLocaleString()}.
                    If no evidence is available for an isoform, we fall back to
                    a rule-based estimate.
                  </>
                ) : (
                  <>No evidence loaded. Showing rule-based estimate only.</>
                )}
              </div>
            </CardContent>
          </Card>

          {/* Right: CYP2E1 Screening (Detailed) */}
          <Card className="card-glow bg-card border-border">
            <CardContent className="p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
